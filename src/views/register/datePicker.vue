...
<script>
  export default {
    ...
    methods: {
          ...
          for (let i2 = 0; i2 < 10; i2++) { // !分成10段来偏移，这是执行运动轨迹,动效的代码
            setTimeout(() => {
              if (i2 === 9) { // ! 前9项都是执行运动，这个是到达目的值了
                ...
                if (i === 0 || i === 1) { // !如果选择的是年份、月份
                  let [year, month] = this.getValueArr();
                  this.year = year;
                  this.month = month;
                  this.setDays();
                  this.arrSelf = [this.years, this.months, this.days];
                  this.initDistance();
                }
              } else {
      ...
      initArr() {
        let time = this.defaultTime.length ? new Date(this.defaultTime.join('/')) : new Date();
        this.year = time.getFullYear();
        this.month = Number(time.getMonth()) + 1;
        this.day = time.getDate();
        this.valueSelf = [this.year, this.month, this.day];
